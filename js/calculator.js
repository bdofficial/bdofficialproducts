var my={} 
function calcMain(imode){this.version='0.934';this.w=350;this.h=300;this.mode=typeof imode!=='undefined'?imode:'Multiply';var id="calc";this.keys=["7","8","9","\u00d7","\u00f7","4","5","6","+","\u2212","1","2","3","(",")","0",".","^","\u2190","C"];this.keysn=this.keys.length;this.fkeys=[];var s='';s+='<div style="position:relative; max-width:400px; min-height:'+h+'px; border: none; border-radius: 20px; margin:auto; display:block;">';s+='<textarea disabled id="u" style="width:98%;pointer-events: none;background-color:white; height:60px; font: 25px Arial; color: black; text-align:center; z-index:2; border-radius: 10px;" onkeyup="userKey(event)" autofocus >&nbsp;</textarea>';s+='<div style="display: block; position:relative; z-index:5;">';s+='<div id="a" style="display: inline-block; margin-top:-2px; width:98%; padding:3px; font: bold 20px Arial; background-color:#532DD0; color: white; border: 1px solid #bbb; border-radius: 10px; text-align: center;" >&nbsp;</div>';s+='<button id="rad" style="position: absolute; right:0px; top: 28px; color: #000aae; font-size: 14px;" class="togglebtn"  onclick="toggleRad()" >rad</button>';s+='</div>';s+=getKeyHTML();s;s+=getfKeyHTML();s+='</div>';document.write(s);hists=[];lastHist="?";lastHistTime=0;this.p=new Parser();this.radsQ=true;toggleRad();doCalc(false);document.getElementById("u").focus();}
function tests(){var exs=['3-1.1e3/1e2 - e + e^2- 2.3 -45','1E4','3-1e-4','-2.2e-4-3','3*sqrt(2)','pi/2','2pi','e^2','2e','27','asin(1)','$3,600.5/2','1+2-3','3(7+3)','1/sqrt(2)','sqrt(-1)','12+2*3','abs(-3.2)','floor(-3.2)','ceil(-3.2)','sign(-3.2)','round(-3.2)','sinh(-1.1)','2.e2','2^0.5','2^-0.5','2^-.5','3.1*-3','3.1*-3.1','1e-4','1e4 - 1e-4','1e4 + 1e-4','1/sqrt(2)','exp(1)','1.2345*1e14','pi*1e12','pi*1e15','pi*1e-12'];for(var i=0;i<exs.length;i++){var ex=exs[i];this.p.newParse(ex);var num=this.p.getVal();num=fmtRP(num);console.log("ex:",ex,'==>',p.rootNode.walkFmt(),'==>',p.getVal(),'==>',num);}}
function toggleRad(){this.radsQ=!this.radsQ;var inddiv=document.getElementById('rad');if(this.radsQ){inddiv.innerHTML='';}else{inddiv.innerHTML='';}
doCalc(false);return;}
...
s+=lineEnd;return s;}
function doKey(i){var key;if(i<this.keysn){key=this.keys[i];}else{key=this.fkeys[i-this.keysn];}
console.log("doKey",i,key);var simples=["1","2","3","4","5","6","7","8","9","0","(",")","+","\u00d7","\u00f7","\u2212",".",'^','i'];if(simples.indexOf(key)>=0){pushU(key);}else{switch(key){case "=":var x=doCalc(true);break;case "C":document.getElementById('u').value='';document.getElementById('a').innerHTML='&nbsp;';break;case "1/x":aroundU('(1/(','))');break;case "\u00B1":aroundU('(-(','))');break;case "sin":aroundU('sin(',')');break;case "cos":aroundU('cos(',')');break;case "tan":aroundU('tan(',')');break;case "sin<sup>-1</sup>":aroundU('asin(',')');break;case "cos<sup>-1</sup>":aroundU('acos(',')');break;case "tan<sup>-1</sup>":aroundU('atan(',')');break;case "e<sup>x</sup>":aroundU('exp(',')');break;case "ln":aroundU('ln(',')');break;case "\u221a":aroundU('sqrt(',')');break;case "x\u00b2":aroundU('(',')^2');break;case "x\u00b3":aroundU('(',')^3');break;case "rad":toggleRad(i);break;case "pi":pushU('pi');break;case "\u2190":var s=document.getElementById('u').value;s=s.substring(0,s.length-1);document.getElementById('u').value=s;break;default:}}}
...
function aroundU(s1,s2){var s=document.getElementById('u').value;var ops=["+","-","\u2212",'*',"\u00d7",'/',"\u00f7",'^'];for(var i=s.length-1;i>=0;i--){if(ops.indexOf(s.charAt(i))>-1)break;}
var sNew=''
...
function doCalc(updateUQ){var f=document.getElementById('u').value;this.p.radiansQ=this.radsQ;this.p.newParse(f);var num=this.p.getVal();num=fmtRP(num);if(num==Number.POSITIVE_INFINITY)this.p.errMsg="undefined";if(num==Number.NEGATIVE_INFINITY)this.p.errMsg="undefined";if(this.p.errMsg.length>0){document.getElementById('a').innerHTML='&nbsp;';}else{if(updateUQ){document.getElementById('u').value='';}
...
function fmtRP(num){if(num==Number.POSITIVE_INFINITY)return "undefined";if(num==Number.NEGATIVE_INFINITY)return "undefined";var s=(+num).toPrecision(15);if(s.indexOf(".")>0&&s.indexOf('e')<0){s=s.replace(/0+$/,'');}
if(s.charAt(s.length-1)=='.'){s=s.substr(0,s.length-1);}
return s;}
class Parser{constructor(){this.operators="+-*(/),^.";this.rootNode=null;this.tempNode=[];this.Variable="x";this.errMsg="";this.radiansQ=true;this.vals=[];for(var i=0;i<26;i++){this.vals[i]=0;}
this.reset();}
...
newParse(s){this.reset();s=s.replace(/[^\w/\.\(\)\[\]\+\-\^\%\&\;\*\!\u2212\u00F7\u00D7\u00B2\u00B3\u221a]/gi,'');s=s.split("x").join("*");s=s.split("[").join("(");s=s.split("]").join(")");s=s.split("&nbsp;").join("");s=s.split("&mult;").join("*");s=s.split("&divide;").join("/");s=s.split("&minus;").join("-");s=s.replace(/\u2212/g,'-');s=s.replace(/\u00F7/g,'/');s=s.replace(/\u00D7/g,'*');s=s.replace(/\u00B2/g,'^2');s=s.replace(/\u00B3/g,'^3');s=s.replace(/\u221a([0-9\.]+)/g,'sqrt($1)');s=s.replace(/\u221a\(/g,'sqrt(');s=this.fixPercent(s);s=this.fixENotation(s);s=this.fixParentheses(s);s=this.fixUnaryMinus(s);s=this.fixFactorial(s);s=this.fixImplicitMultply(s);this.rootNode=this.parse(s);}
...
bits.push(s.substr(stt));s=bits.join('');return s;}
fixFactorial(s){var currPos=1;do{var chgQ=false;var fPos=s.indexOf("!",currPos);if(fPos>0){var numEnd=fPos-1;var numStt=numEnd;var cnum=s.charAt(numStt);if(cnum=='n'){}
else{do{var cnum=s.charAt(numStt);numStt--;}while(cnum>="0"&&cnum<="9");numStt+=2;}
...
bits.push(s.substr(stt));s=bits.join('');return s;}
fixParentheses(s){var sttParCount=0;var endParCount=0;for(var i=0;i<s.length;i++){if(s.charAt(i)=="(")
sttParCount++;if(s.charAt(i)==")")
endParCount++;}
while(sttParCount<endParCount){s="("+s;sttParCount++;}
while(endParCount<sttParCount){s+=")";endParCount++;}
return(s);}
fixUnaryMinus(s){var x=s+"\n";var y="";var OpenQ=false;var prevType="(";var thisType="";for(var i=0;i<s.length;i++){var c=s.charAt(i);if((c>="0"&&c<="9")||c=='.'){thisType="N";}
else{if(this.operators.indexOf(c)>=0){if(c=="-"){thisType="-";}
else{thisType="O";}}
else{if(c=="."||c==this.Variable){thisType="N";}
else{thisType="C";}}
if(c=="("){thisType="(";}
if(c==")"){thisType=")";}}
...
return(y);}
fixImplicitMultply(s){var x=s+"\n";var y="";var prevType="?";var prevName="";var thisType="?";var thisName="";for(var i=0;i<s.length;i++){var c=s.charAt(i);if(c>="0"&&c<="9"){thisType="N";}
else{if(this.operators.indexOf(c)>=0){thisType="O";thisName="";}
else{thisType="C";thisName+=c;}
if(c=="("){thisType="(";}
if(c==")"){thisType=")";}}
...
var isParam=false;if(bracStt==0){isParam=false;}
else{var prefix=s.substr(bracStt-1,1);isParam=this.operators.indexOf(prefix)<=-1;}
if(!isParam){this.tempNode.push(this.parse(s.substr(bracStt+1,bracEnd-bracStt-1)));return this.parse(s.substr(0,bracStt)+"$"+(this.tempNode.length-1).toString()+s.substr(bracEnd+1,s.length-bracEnd-1));}
else{var startM=-1;for(var u=bracStt-1;u>-1;u--){var found=this.operators.indexOf(s.substr(u,1));if(found>-1){startM=u;break;}}
...
var k;var k1=s.lastIndexOf("+");var k2=s.lastIndexOf("-");if(k1>-1||k2>-1){if(k1>k2){k=k1;var nnew=new MathNode("op","add",this.radiansQ);nnew.addchild(this.parse(s.substr(0,k)));nnew.addchild(this.parse(s.substr(k+1,s.length-k-1)));return nnew;}
else{k=k2;nnew=new MathNode("op","sub",this.radiansQ);nnew.addchild(this.parse(s.substr(0,k)));nnew.addchild(this.parse(s.substr(k+1,s.length-k-1)));return nnew;}}
k1=s.lastIndexOf("*");k2=s.lastIndexOf("/");if(k1>-1||k2>-1){if(k1>k2){k=k1;nnew=new MathNode("op","mult",this.radiansQ);nnew.addchild(this.parse(s.substr(0,k)));nnew.addchild(this.parse(s.substr(k+1,s.length-k-1)));return nnew;}
else{k=k2;nnew=new MathNode("op","div",this.radiansQ);nnew.addchild(this.parse(s.substr(0,k)));nnew.addchild(this.parse(s.substr(k+1,s.length-k-1)));return nnew;}}
...
if(isNumeric(s)){return new MathNode("real",s,this.radiansQ);}
else{if(s.length==0){return new MathNode("real","0",this.radiansQ);}
else{this.errMsg+="'"+s+"' is not a number.\n";return new MathNode("real","0",this.radiansQ);}}}}
...
switch(this.op){case "sqrt":val=Math.sqrt(lhs);break;case "sin":val=Math.sin(lhs/angleFact);break;case "cos":val=Math.cos(lhs/angleFact);break;case "tan":val=Math.tan(lhs/angleFact);break;case "asin":val=Math.asin(lhs)*angleFact;break;case "acos":val=Math.acos(lhs)*angleFact;break;case "atan":val=Math.atan(lhs)*angleFact;break;case "sinh":val=(Math.exp(lhs)-Math.exp(-lhs))/2;break;case "cosh":val=(Math.exp(lhs)+Math.exp(-lhs))/2;break;case "tanh":val=(Math.exp(lhs)-Math.exp(-lhs))/(Math.exp(lhs)+Math.exp(-lhs));break;case "exp":console.log('exp',lhs,Math.exp(lhs))
val=Math.exp(lhs);break;case "log":val=Math.log(lhs)*Math.LOG10E;break;case "ln":val=Math.log(lhs);break;case "abs":val=Math.abs(lhs);break;case "deg":val=lhs*180.0/Math.PI;break;case "rad":val=lhs*Math.PI/180.0;break;case "sign":if(lhs<0){val=-1;}else{val=1;}
